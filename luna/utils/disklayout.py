...
def brief(raw: str | bytes | None) -> str | None:
    """Render a stored layout as a short block for ``luna node|group show``.

    Returns ``None`` when nothing is declared (a legal state: ``install_mode=auto``
    then falls back to a RAM root), the summary otherwise, and raises
    :class:`DisklayoutError` when a layout is present but unusable.

    Shape mirrors how ``show`` already renders interfaces: an unindented header per
    set, its volumes indented beneath. Volumes share ONE line rather than getting
    one each, and that is load-bearing rather than cosmetic -- ``show`` passes this
    through a length limiter that keeps only the first three lines of anything
    longer, so a per-volume rendering would silently lose most of itself. The
    per-volume detail (provider, volume names) lives in ``showdisklayout``.
    """
    if raw is None:
        return None
    text = _decode(raw) if isinstance(raw, bytes) else str(raw)
    if not text.strip():
        return None
    try:
        layout = json.loads(text)
    except ValueError as error:
        raise DisklayoutError(f'not valid JSON: {error}') from error
    if not isinstance(layout, dict):
        raise DisklayoutError('top-level must be an object')
    sets = layout.get('sets')
    if not isinstance(sets, list):
        raise DisklayoutError('sets must be a list')
    lines: list[str] = []
    for a_set in sets:
        if not isinstance(a_set, dict):
            raise DisklayoutError('each set must be an object')
        ...
        for vol in volumes:
            if not isinstance(vol, dict):
                raise DisklayoutError('each volume must be an object')
